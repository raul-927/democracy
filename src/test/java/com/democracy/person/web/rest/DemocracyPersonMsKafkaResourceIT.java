...
import com.democracy.person.config.KafkaProperties;
...
class DemocracyPersonMsKafkaResourceIT {
    ...
    void setup() {
        ...
        DemocracyPersonMsKafkaResource kafkaResource = new DemocracyPersonMsKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/democracy-person-ms-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/democracy-person-ms-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
